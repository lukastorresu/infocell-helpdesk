<x-app-layout>
    <div class="container mx-auto mt-10 p-5 bg-white rounded-lg shadow-md max-w-lg">
        <h1 class="text-2xl font-bold mb-5 text-gray-700">Editar Tipo de Chamado</h1>

        <form action="{{ route('tipos-chamado.update', $tipoChamado->id) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="nome" class="block text-gray-700 font-semibold mb-2">Nome do Tipo</label>
                <input type="text" id="nome" name="nome" value="{{ $tipoChamado->nome }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg" required>
            </div>

            <div class="mb-4">
                <label for="descricao" class="block text-gray-700 font-semibold mb-2">Descrição (Opcional)</label>
                <input type="text" id="descricao" name="descricao" value="{{ $tipoChamado->descricao }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
            </div>
